...
function Sidebar({
    ...
    useEffect(() => {
        const filterListWork = listEvents?.filter(
            (e) => e.raw.eventType === 'target' && e.raw.isImportain,
        );
        ...
        <div style={{ position: 'relative' }}>
            <div className={visitable ? cx('sidebar', 'visitable') : cx('sidebar')}>
                ...
                <div className={cx('nav')}>
                    <div
                        className={cx('nav-item', isToday ? 'active' : null)}
                        onClick={() => {
                            setIsTab('/');
                        }}
                    >
                    ...
                    <div
                        className={cx('nav-item', isImportant ? 'active' : null)}
                        onClick={() => setIsTab('/important')}
                    >
                        <FolderOutlined />
                        <span className={cx('item-title')}>Quan trọng</span>
                    </div>
                    <div
                        className={cx('nav-item', isSharewith ? 'active' : null)}
                        onClick={() => setIsTab('/assigned')}
                    >
                        <FolderOutlined />
                        <span className={cx('item-title')}>Công việc được giao</span>
                    </div>
                    <div
                        className={cx('nav-item', isListTask ? 'active' : null)}
                        onClick={() => setIsTab('/alltask')}
                    >
                        <FolderOutlined />
                ...
                <div className={cx('listwork')} onClick={() => setIsOpenListWork(!isOpenListWork)}>
                    ...
                    <div className={cx('work-list')}>
                        {isOpenListWork &&
                            listWork?.map((e, index) => (
                                <div
                                    className={cx('work-item')}
                                    onClick={() => navigate(`/overview?eventId=${e.id}`)}
                                >
                                    <div
                                        className={cx('work-item-checkbox')}
